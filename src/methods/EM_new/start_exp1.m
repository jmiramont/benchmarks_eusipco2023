...
methods_name = {'Brevdo SST', 'Brevdo STFT', 'SSA', ...   %% 1,2,3
   'Proposed \beta=1, Beta div.', 'Proposed KL div.', 'Proposed \alpha=0.9, Renyi div.', ... %% 4,5,6
   sprintf('Oracle \\Gamma=%.2f',gamma_mask), 'EM', 'Nils'     };   %% 7, 8, 9

%methods_to_use = [4 5 6 7 1 2 3];   % insert here the indices of the methods to compare (names above)
methods_to_use = [1]; %[1 2 6];   % insert here the indices of the methods to compare (names above)
...
%% 4,5,6saveas(gcf, sprintf('figs/sst.eps'), 'epsc'); 
...
for ind_met = 1:nb_methods
  ...
  %% Bayesian
        ...
        case 8 %% EM
          ...
          [W_out,P,ind0]=Mod_Estim_W_EM(Y',Fc,Ncomp,step_Nx,reg_mu,c,cl, 0);
