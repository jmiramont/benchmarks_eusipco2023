...
methods_name = {'Brevdo SST', 'Brevdo STFT', 'SSA', 'Proposed \beta=1, Beta div.', ...
    'Proposed KL div.', 'Proposed \alpha=0.9, Renyi div.', sprintf('Oracle \\Gamma=%.2f',gamma_mask), ...
    'EM'};
%methods_to_use = [4 5 6 7 1 2 3];   % insert here the indices of the methods to compare (names above)
methods_to_use = [8] %[1 2 3 7];   % insert here the indices of the methods to compare (names above)
...
for ind_met = 1:nb_methods
  ...
  %% Bayesian
        ...
        case 8   %% EM (new proposed)
          ...
          seuil = 1e-2;
          x_hat = zeros(N,Ncomp);
          [ y_hat, mask ] = EM_estim_multi(tfr, L, nb_comp, seuil, c, cl, step_Nx, ifplot  );
          for c = 1:Ncomp
            x_hat(:,c) = real(rectfrgab(tfr .* mask(:,:,c), L, M));
          end          
