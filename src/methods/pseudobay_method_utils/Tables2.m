...
%  of the RQF with a SNR = -15dB to reproduce the results depicted in Table 2
...
% Amplitude modulated
amp(:,1)=1:-0.5/(N-1):0.5;
amp(:,2)=0.5.*cos(2*pi*(0:N-1)/(2*N/3)) + 1;
amp(:,3)=(0.5+(((-249:250)/250).^2));

X(:,1) = amp(:,1).*real(fmconst(N, 0.1));
X(:,2) = amp(:,2).*real(fmlin(N,0.13,0.3));
X(:,3) = amp(:,3).*real(fmsin(N,0.3,0.45,320,1,0.3,+1));
