function [W_out,P,Mu_out]=Mod_Estim_W_EM_multi(Y,Fct,Ns,step_Nx,reg_mu,c,step_r,step_v,ifplot,bolpol)
...
%     disp(['EM iteration : ',num2str(m_compt)])
    ...
    if (((m_compt>=10) && (derr<1e-5)) || m_compt==15)  % Begin the last 5 iterations
        Stopping_EM = 1;
    end
    if Stopping_EM == 1
        W_out=W_out+wc;
